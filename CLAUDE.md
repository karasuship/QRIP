...
- ✅ /signal/qqq・/signal/vt 詳細ページ（EtfSignalPage 共用コンポーネント）
...
## 次にやること（2026-06-28 更新）

### UI改善タスク（サイト全体審査 2026-06-28 より）

優先度順。小さい効果・高い効果から着手。

| # | タスク | 工数 | 効果 | 状態 |
|---|---|---|---|---|
| 1 | nav「検証」→「根拠」に改名（検証とシグナルの混同解消） | 極小 | 中 | ✅ |
| 2 | スクリーナーに件数上限（上位100件表示 + 「絞り込んで件数を減らしてください」表示） | 小 | 大 | ✅ |
| 3 | 試算（/simulate）にプリセットボタン3個（月3万・月5万・月10万） | 小 | 大 | ✅ |
| 4 | ニュースページ冒頭に「今日の結論」一行サマリー追加 | 小 | 中 | ✅ |
| 5 | マイページのウォッチリスト詳細に当日騰落率・52週バー追加 | 小 | 中 | ✅ |
| 6 | ホームの「過去の買い場」「シミュレーション」「検証書庫」「仮説」を折り畳み（初期は開いた状態） | 中 | 大 | ✅ |
| 7 | SP500詳細ページの重要度整理（CRS 6成分ドットを折り畳み・補足情報をセカンダリ扱いに） | 中 | 中 | ✅ |
| 8 | QQQ・VT の詳細ページ（/signal/qqq・/signal/vt）作成 | 大 | 中 | ✅ |

### フェーズ1〜5 — 完了
1〜13 すべて実装済み（2026-06-27 以前）

### フェーズ6 — 後で（優先度低）
- インサイダー取引履歴（Yahoo insiderTransactions、JP株は薄い）
- FOMC声明スコアリング（要研究）
- LINE公式アカウント
- Google AdSense（コンテンツ充実が先）
- 株主優待（irbank リンクのみか手動100銘柄か）
- スクリーナー → シミュレーション連携（実装済みだが改善余地あり）
