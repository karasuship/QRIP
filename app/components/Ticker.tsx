...
export default function Ticker({ initial }: { initial: SignalData }) {
  ...
  return (
    <div className="border-b border-[#16162a] bg-[#0a0a12] py-1.5 overflow-hidden">
      <div className="flex overflow-hidden">
        <div className="flex animate-ticker whitespace-nowrap">
          {doubled.map((item, i) => (
            <span key={i} className="inline-flex items-center gap-2 px-6 border-r border-[#16162a]">
              <span className="font-mono text-[10px] uppercase tracking-widest text-[#2e2e48]">
                {item.label}
              </span>
