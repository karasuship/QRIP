    ...
    <span
      className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 font-mono text-[10px] tracking-wide ${
        active
          ...
          : "border-[#1e1e32] bg-[#0c0c15] text-[#2e2e48]"
      }`}
    >
      <span
        className={`inline-block h-1.5 w-1.5 rounded-full ${
          active ? "bg-red-500" : "bg-[#2e2e48]"
        }`}
      />
      {label}
    </span>
  ...
  return (
    <div className={`rounded-xl border p-3 ${border}`}>
      <div className="flex items-center gap-2">
        ...
        <p className={`text-sm font-medium ${active ? text : "text-[#2e2e48]"}`}>
          {label}
        </p>
        {active && (
          <span className={`ml-auto font-mono text-xs font-semibold ${text}`}>
            発動中
          </span>
        )}
      </div>
      <p className="mt-1 pl-4 font-mono text-[10px] text-[#2e2e48]">{sub}</p>
    </div>
  );
}
...
export default async function SignalPage() {
    ...
    return (
      <div className="min-h-screen bg-[#050508] text-[#e2e8f0]">
        <main className="mx-auto max-w-2xl px-5 py-12">
          <Link href="/" className="font-mono text-xs text-[#64748b] hover:text-[#94a3b8]">
            ← ホームにもどる
          </Link>
          <p className="mt-8 text-sm text-red-400">
            データ取得に失敗しました。しばらくしてから再読み込みしてください。
          </p>
          {error && <p className="mt-2 font-mono text-xs text-[#2e2e48]">{error}</p>}
        </main>
      </div>
    );
  }
  ...
  const statusMap: Record<typeof signalTier, StatusConfig> = {
    ...
    NONE: {
      ...
      border: "border-[#1e1e32] bg-[#0c0c15]",
  ...
  return (
    <div className="min-h-screen bg-[#050508] text-[#e2e8f0]">
      <main className="mx-auto max-w-2xl px-5 py-12">
        <Link href="/" className="font-mono text-xs text-[#64748b] hover:text-[#94a3b8] transition-colors">
          ← ホームにもどる
        </Link>
        ...
        <div className="mt-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 font-mono text-[10px] text-[#2e2e48]">
              {date} · 15分キャッシュ · Yahoo Finance ^GSPC
            </p>
          </div>
          <PushSubscribe />
        </div>
        ...
        {/* メインステータス */}
        <div className={`mt-4 rounded-2xl border p-5 ${st.border}`}>
          <p className="font-mono text-[10px] uppercase tracking-widest text-[#2e2e48] mb-2">
            シグナル状態
          </p>
          <p className="text-xl font-semibold text-[#e2e8f0]">{st.label}</p>
          <p className="mt-1 font-mono text-xs text-[#64748b]">{st.sub}</p>
          <p className={`mt-3 text-sm font-medium ${st.detailColor}`}>
            {st.detail}
          </p>
        </div>
        ...
        <section className="mt-5">
          <p className="mb-2 font-mono text-[10px] uppercase tracking-widest text-[#2e2e48]">
            全シグナル状態（4体制 v3）
          </p>
        ...
        {/* CRS スコア */}
        <section className="mt-5 rounded-xl border border-[#1e1e32] bg-[#0c0c15] p-4">
              ...
              className={`rounded-full px-3 py-1 font-mono text-sm font-bold ${
                crs >= 4
                  ...
                  : crs >= 2
                    ...
                    : "bg-[#16162a] text-[#64748b]"
              }`}
            >
              {crs} / 6
            </span>
          </div>
          <p className="mt-1 mb-3 font-mono text-[10px] text-[#2e2e48]">
        ...
        {/* phi2 v3 条件チェックリスト */}
        <section className="mt-4 rounded-xl border border-[#1e1e32] bg-[#0c0c15] p-4">
          <p className="mb-3 font-mono text-[10px] uppercase tracking-widest text-[#2e2e48]">
            phi2 v3 発動条件
          </p>
          {(
            [
            ...
            <div
              key={label}
              className="flex items-center justify-between border-b border-[#16162a] py-2 last:border-0"
            >
              <span className="flex items-center gap-2 text-sm">
                <span className={`font-mono ${ok ? "text-[#10b981]" : "text-[#2e2e48]"}`}>
                  {ok ? "✓" : "○"}
                </span>
                <span className={ok ? "text-[#e2e8f0]" : "text-[#64748b]"}>{label}</span>
              </span>
              <span className={`font-mono text-sm ${ok ? "text-[#10b981]" : "text-[#2e2e48]"}`}>
                {val}
              </span>
            </div>
          ))}
        ...
        {pastEpisodes.length > 0 && (
          <section className="mt-8 border-t border-[#1e1e32] pt-6">
            <p className="font-mono text-[10px] uppercase tracking-widest text-[#2e2e48]">
              過去2年の phi2 類似事例
            </p>
            ...
            <div className="overflow-hidden rounded-xl border border-[#1e1e32]">
              <table className="w-full text-sm">
                <thead className="border-b border-[#1e1e32] bg-[#090910]">
                  <tr>
                    <th className="px-3 py-2 text-left font-mono text-[9px] uppercase tracking-widest text-[#2e2e48]">日付</th>
                    <th className="px-3 py-2 text-right font-mono text-[9px] uppercase tracking-widest text-[#2e2e48]">当日</th>
                    <th className="px-3 py-2 text-right font-mono text-[9px] uppercase tracking-widest text-[#2e2e48]">ATH 乖離</th>
                    <th className="px-3 py-2 text-right font-mono text-[9px] uppercase tracking-widest text-[#2e2e48]">今日まで</th>
                    <th className="px-3 py-2 text-right font-mono text-[9px] uppercase tracking-widest text-[#2e2e48]">経過</th>
                  </tr>
                </thead>
                <tbody>
                  {pastEpisodes.slice(0, 10).map((ep) => (
                    <tr key={ep.date} className="border-t border-[#16162a]">
                      <td className="px-3 py-2 font-mono text-xs text-[#94a3b8]">{ep.date}</td>
                      <td className="px-3 py-2 text-right font-mono text-xs text-[#ef4444]">
                        {pct(ep.dayRet)}
                      </td>
                      ...
                      <td className={`px-3 py-2 text-right font-mono text-xs font-semibold ${
                        ep.retToDate > 0 ? "text-[#10b981]" : "text-[#ef4444]"
                      }`}>
                        {pct(ep.retToDate)}
                      </td>
                      <td className="px-3 py-2 text-right font-mono text-[10px] text-[#2e2e48]">
                        {ep.daysAgo < 30
                          ? `${ep.daysAgo}日前`
        ...
        <section className="mt-8 border-t border-[#1e1e32] pt-6">
          <p className="font-mono text-[10px] uppercase tracking-widest text-[#2e2e48]">
            過去 30 日の phi2 v3 候補日
          </p>
          <p className="mt-1 mb-3 text-xs text-[#64748b]">
            ATH −10% · 当日 −2% · vol &gt; 25% を満たした日
          </p>
          {history.length === 0 ? (
            <p className="font-mono text-xs text-[#2e2e48]">
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-[#1e1e32] bg-[#090910]">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right font-mono text-[9px] uppercase tracking-widest text-[#2e2e48]">当日</th>
                    <th className="px-3 py-2 text-right font-mono text-[9px] uppercase tracking-widest text-[#2e2e48]">ATH 乖離</th>
                    <th className="px-3 py-2 text-right font-mono text-[9px] uppercase tracking-widest text-[#2e2e48]">CRS</th>
                    <th className="px-3 py-2 text-right font-mono text-[9px] uppercase tracking-widest text-[#2e2e48]">phi2 v3</th>
                  </tr>
                </thead>
                <tbody>
                  {history.map((h) => (
                    <tr key={h.date} className="border-t border-[#16162a]">
                      <td className="px-3 py-2 font-mono text-xs text-[#94a3b8]">{h.date}</td>
                      <td className="px-3 py-2 text-right font-mono text-xs text-[#ef4444]">
                        {pct(h.dayRet)}
                      </td>
                      <td className="px-3 py-2 text-right font-mono text-xs text-[#64748b]">{pct(h.athDd)}</td>
                      <td className="px-3 py-2 text-right font-mono text-xs">
                        {h.crs !== null ? (
                          <span className={h.crs >= 2 ? "text-amber-400" : "text-[#2e2e48]"}>
                            {h.crs}/6
                          </span>
                        ) : (
                          <span className="text-[#2e2e48]">—</span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-right font-mono text-xs">
                        {h.phi2v3 ? (
        ...
        <p className="mt-8 font-mono text-[10px] leading-6 text-[#2e2e48]">
          ...
          phi2 v3:{" "}
          <span className="text-[#1e1e32]">decisions/0021</span>（TEST Z=+8.65）·
          HYG-8%:{" "}
          <span className="text-[#1e1e32]">decisions/0016</span>（TEST Z=+9.42）·
          B4:{" "}
          <span className="text-[#1e1e32]">decisions/0018</span>（TEST Z=+8.29）。
          これは投資助言ではありません。
        </p>
      </main>
    </div>
