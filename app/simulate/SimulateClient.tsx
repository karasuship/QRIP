...
export default function SimulateClient({ initialStock }: { initialStock?: InitialStock }) {
  ...
  const [assets, setAssets] = useState<Asset[]>(() => {
    if (initialStock) {
      const divY = initialStock.divYield;
      return [
        {
          id: crypto.randomUUID(),
          name: initialStock.name,
          pct: 100,
          totalRet: Math.round((divY + 3.5) * 10) / 10,
          divYield: divY,
          drip: false,
        },
      ];
    }
    return [newAsset(0, 50), newAsset(2, 30), newAsset(1, 20)];
  });
